...
import { getArrWork, xoaCongViec } from '../../../redux/reducer/workReducer';
const { Search } = Input;
 ...
 function Gig(props) {
  ...
  const columns = [
    {
      ...
      render : (text,work,index) =>{
        return <Fragment key={text}>
          ...
          }} style={{width:"100px", height:"100px"}} src={work.image} alt="..." />
        </Fragment>
      },
      width: "10%",
    ...
    },
    {
      ...
      render: (text, object) => {
        return (
          <span>
            {object.proServices === true ? <i className="text-success fa fa-check"></i> : ""}
          </span>
        );
      },
      width: "calc(100% / 7)",
    },
    {
      ...
      render: (text,work) =>{
        return <Fragment>
          <NavLink style={{fontSize: "20px", fontWeight:"bold"}} to={`/admin/gig/editgig/${work._id}`}>
  ...
  return (
    <section className='users'>
      <h1 className='text-center mb-5' style={{fontSize: "30px", fontWeight:"bold", color:"black"}}>Quản Lý Công Việc</h1>
      <button onClick={() =>{
        history.push("/admin/gig/addgig")
      }} className="btn__login btn btn-success">Thêm Công Việc</button>
      <Search
      className='my-3'
