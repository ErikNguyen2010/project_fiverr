        ...
        <h1 className='text-center mb-5' style={{fontSize: "30px", fontWeight:"bold", color:"black"}}>Thêm Công Việc</h1>
             <Form
               onSubmitCapture={formik.handleSubmit}
                ...
                size={componentSize}
                >
                <Form.Item name="name" label="Job Name " rules={[
                      {
                        type: 'name',
                    ...
                    <Input onChange={formik.handleChange} name='name' placeholder='Name' />
              </Form.Item>
                <Form.Item label="Rating">
                      <InputNumber name='rating' onChange={(value) =>{
                      formik.setFieldValue("rating",value)
                    }}  min={1} max={10} />{" "}
                  </Form.Item>

                <Form.Item label="Price">
                    <InputNumber name='price' onChange={(value) =>{
                      formik.setFieldValue("price",value)
                    }} min={0} max={100000}/>{" "}
                </Form.Item>

                <Form.Item label="ProServices" valuePropName="checked">
                    <Switch name="proServices" onChange={handleChangeSwitch("proServices")}  />
                </Form.Item>

                <Form.Item label="Local Sellers" valuePropName="checked">
                    <Switch name="localSellers" onChange={handleChangeSwitch("localSellers")}  />
                </Form.Item>

                <Form.Item label="Online Sellers" valuePropName="checked">
                    <Switch name="onlineSellers" onChange={handleChangeSwitch("onlineSellers")}  />
                </Form.Item>

                <Form.Item label="Delivery Time" valuePropName="checked">
                    <Switch name="deliveryTime" onChange={handleChangeSwitch("deliveryTime")}  />
                </Form.Item>

                <Form.Item label="Image">
                    <input name="image" type="file" accept='image/png,image/jpg,image/jpeg,image/git' onChange={handleChangeFile}/>
                    <br/>
